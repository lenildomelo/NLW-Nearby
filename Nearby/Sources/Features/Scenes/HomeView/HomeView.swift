...
class HomeView: UIView {
    ...
    func updateFilterButtons(with categories: [Category], action: @escaping (Category) -> Void) {
        ...
        for (index, category) in categories.enumerated() {
            ...
            if category.name == "Alimentação" {
                updateButtonSelectio(button: button)
            }
    ...
    private func updateButtonSelectio(button: UIButton) {
    ...
    private func filterButtonTapped(_ sender: UIButton) {
        let selectedCaregory = categories[sender.tag]
        updateButtonSelectio(button: sender)
        filterButtonAction?(selectedCaregory)
    }
